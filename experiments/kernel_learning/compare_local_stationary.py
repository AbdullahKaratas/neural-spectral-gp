...
from nsgp.models import DKLGP, FactorizedSpectralDensityNetwork, NeuralGSMGP, StandardGP
...
def compare_kernels(
    ...
    # True posterior
    axes1[0].plot(
        X_plot, mean_true.detach().numpy(), "k-", linewidth=2, label="True Posterior"
    )
    ...
    axes1[0].scatter(
        X_train_plot, y_train_plot, c="red", s=20, alpha=0.3, label="Training data"
    )
    ...
    axes1[1].plot(
        X_plot, mean_sdn.detach().numpy(), "b-", linewidth=2, label="F-SDN Posterior"
    )
    ...
    axes1[2].plot(
        X_plot, mean_gp.detach().numpy(), "g-", linewidth=2, label="RBF Posterior"
    )
    ...
    axes1[3].plot(
        X_plot,
        mean_ngsm.detach().numpy(),
        color="orange",
        linewidth=2,
        label="Neural-GSM Posterior",
    )
    ...
    print("Results:")
    print(
        f"{'Method':<15} | {'K-error':>10} | {'KL':>8} | {'MLL':>8} | {'noise_var':>10}"
    )
    for i, method in enumerate(methods):
        ...
        m = metrics[method]
        print(
            f"{method:<15} | {rel_error:>10.2%} | {m['kl']:>8.3f} | "
            f"{m['mll']:>8.3f} | {m['noise_var']:>10.2e}"
        )
