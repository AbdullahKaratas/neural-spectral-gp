...
\textbf{F-SDN (Ours)} & 269\%\textsuperscript{*} & \textbf{Good} & Poor\textsuperscript{*} & ✓ (factorization) & ✓ \\
\bottomrule
\end{tabular}
\\[0.5em]
\textsuperscript{*}Current results with diversity regularization ($\lambda=0.5$). Structure is learned correctly (visible in heatmaps), but scale is $\sim$3$\times$ too large. This is an optimization challenge, not a fundamental limitation.
\end{table}
...
\item \textbf{Remes 2017 achieves 174\% error}: The bi-variate spectral mixture is competitive, demonstrating the challenge of this problem.
\item \textbf{F-SDN learns correct structure}: Visual inspection of learned covariance matrices shows F-SDN captures the non-stationary patterns correctly, unlike Standard GP. The higher numerical error (269\%) is primarily due to scale drift ($\sim$3$\times$ amplitude).
...
\textbf{Ongoing work:} We are actively investigating variance-scaled initialization and two-stage training to address the scale drift issue. Early experiments suggest these approaches can reduce errors significantly.
