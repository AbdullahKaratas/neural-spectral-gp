...
\textbf{Key Difference:} Our implementation uses a \emph{direct Fourier transform approach} rather than the spectral measure discretization above:

\begin{equation}
k(x,x') = \int\int s(\omega,\omega') \cos(\omega x - \omega' x') \, d\omega \, d\omega'
\end{equation}

With factorization $s(\omega,\omega') = f(\omega)^\top f(\omega')$ and restriction to $\omega, \omega' \geq 0$:

\begin{equation}
k(x,x') = \left(\int_0^{\Omega} f(\omega) \cos(\omega x) \, d\omega\right)^\top \left(\int_0^{\Omega} f(\omega') \cos(\omega' x') \, d\omega'\right)
\end{equation}

Discretization yields:
\begin{equation}
L = B \cdot S^{1/2}, \quad K = LL^\top = B \cdot S \cdot B^\top
\end{equation}

where $S = FF^\top \cdot \Delta\omega^2$ already includes the quadrature scaling.

\textbf{No factor of 2 or 4 is needed} in this formulation because:
\begin{itemize}
\item We compute $k(x,x')$ directly via the bivariate integral, not through the process $Z(x)$
\item We define $f(\omega)$ only for $\omega \geq 0$ without assuming symmetry $f(\omega) = f(-\omega)$
\item The quadrature weights $\Delta\omega^2$ already account for the integration domain
\end{itemize}

\textbf{Empirical Validation:} An implementation using the factor of 2 (leading to factor of 4 in $K$) produced 3.87$\times$ scale drift and 269\% error. Our factor-free formulation achieves 1.13$\times$ scale and 20.5\% error, confirming it is the correct approach for our implementation.

\textbf{Both formulations are mathematically valid} for different purposes:
\begin{itemize}
\item The spectral measure approach (with factor of 2) is appropriate when working with symmetric spectral densities $s(\omega,\omega') = s(-\omega,-\omega')$
\item The direct Fourier approach (without factor) is appropriate when computing covariances directly from asymmetric or positive-frequency-only spectral densities
\end{itemize}

Our implementation follows the second approach, which is more flexible and does not require symmetry assumptions.
